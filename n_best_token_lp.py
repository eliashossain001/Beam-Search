...
#             for j in best_k:
#                 candidate = [seq + [j], score + tf.math.log(tf.constant(row[j]))]
    ...
    for i, row in enumerate(data):
        ...
        for seq, seq_log_probs, score in sequences:
            for j in range(len(row)):
                ...
                # compute score
                length_penalty = np.power((5.0 + len(candidate_seq)) / 6.0, 0.65)
                next_token_log_prob = np.log(row[j])
                candidate_score = sum(candidate_seq_log_probs) / len(candidate_seq_log_probs) / length_penalty + next_token_log_prob
                #candidate_score= sum(candidate_seq_log_probs) / len(candidate_seq_log_probs) /length_penalty + next_token_log_prob 
...
beam_search_results = beam_search_decoder(df, beam_width=20, n_best=5)
